    ...
    <div className="cards-display grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-8">
      {cards.map((card, index) => {
        ...
        return (
          <div 
            key={index}
            className={`flex flex-col justify-center items-center px-4 py-6 text-white text-center bg-zinc-600 border-2 border-zinc-800 rounded-xl cursor-pointer transform transition-transform duration-100 hover:scale-105 hover:border-slate-600 hover:bg-zinc-900 ${hoverTextColorClass}`}
          >
